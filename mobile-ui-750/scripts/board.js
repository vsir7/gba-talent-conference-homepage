import { PAGES } from './page-registry.mjs?v=20260903-5';
import { pageHash, pagePreviewPath, resolvePageId } from './board-core.mjs?v=20260903-5';

const PAGE_ASSET_VERSION = '20260903-5';
...
function selectPage() {
  ...
  frame.src = pagePreviewPath(page.file, PAGE_ASSET_VERSION);
  if (location.hash !== pageHash(pageId)) history.replaceState(null, '', pageHash(pageId));
}

frame.addEventListener('load', () => {
  const pathname = new URL(frame.src).pathname;
  const current = PAGES.find((page) => pathname.endsWith(`/pages/${page.file}`));
