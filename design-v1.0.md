...
> 归档说明：本文件保留紫色标题调整前的 V1.0 规则，不再随新版更新；配套机器 Token 冻结于 `design-system/tokens/v1.0/`。
...
- `2.1.0` 明确采用 `light-only` 浅色主题；不得只为跟随系统偏好而临时反转颜色或局部实现深色模式。
...
1. **蓝紫只强调关键行为和品牌身份。** 大面积正文使用深色，不把所有文字都做成品牌蓝。
...
| 4 | 我的报名详情 | 用户身份头图、审核状态卡、键值信息表、危险操作 | 用户身份、成功状态、信息行、底部操作层级 | 标题使用黑色、内容使用蓝色；统一为中性标题 + 品牌色强调 |
...
机器可读 Token 以 [design-tokens.json](/Users/tongyuhu/Documents/ChatGPT/dahui/design-system/tokens/v1.0/design-tokens.json) 为唯一事实源：

- [design-tokens.css](/Users/tongyuhu/Documents/ChatGPT/dahui/design-system/tokens/v1.0/design-tokens.css) 由 JSON 自动生成，供 H5/移动 Web 使用。
- [design-tokens.ts](/Users/tongyuhu/Documents/ChatGPT/dahui/design-system/tokens/v1.0/design-tokens.ts) 由 JSON 自动生成，供 TypeScript/组件库使用。
- [build-tokens.mjs](/Users/tongyuhu/Documents/ChatGPT/dahui/design-system/tokens/v1.0/build-tokens.mjs) 负责生成和漂移检查。
...
- 本次从 `1.1.0` 升级到 `2.0.0`：默认正文、触控目标、字阶命名和基础 Token 均发生迁移，按不兼容升级管理。
- `2.1.0` 新增兼容矩阵、浅色主题范围、国际化/性能发布政策和补充组件 Token，不改变 `2.0.0` 已有 Token 的含义。
...
| v1 项 | v2 处理 | 迁移动作 |
|---|---|---|
| `typography.bodyMedium = 15/24` | `16/24` | H5 正文和输入统一改用 `bodyMedium` |
| `typography.bodyLarge = 16/26` | `17/26` | 仅强调正文使用；普通正文不要整体放大 |
| `typography.cardTitle = 17/24` | 默认 `18/26` + `cardTitleCompact 17/24` | 紧凑列表显式切换 compact |
| `typography.label` | `labelMedium 14/20` + `labelSmall 13/18` | 按控件标签或紧凑状态分别替换 |
| `size.touchTarget = 44` | H5 默认 `48`，新增 `touchTargetIosMin = 44` | H5 扩大热区；44 只保留原生 iOS 兼容语义 |
| `size.control.xs = 28` | `component.microAction.visualSize = 28` | 微型视觉元素放入 48px 父级热区 |
| `size.control.sm = 36` | `control.compact = 36`，`control.sm = 40` | 低频紧凑控件显式使用 compact |
| `size.icon.xs = 14` | 基础图标从 16px 起；标签图标使用 `component.tag.iconSize = 14` | 移除全局 14px 图标档 |
| `space.2 / 6 / 10` | 从全局间距移除 | 按组件 Token 迁移，不得继续作为页面间距 |
| `radius.2xl = 24` | `component.hero.marketingRadius = 24` | 只允许大型营销容器使用 |
| `size.searchBar = 44` | 默认 48；新增 `searchBarCompact = 44` | 普通搜索升级为 48，复合工具栏才显式 compact |
...
| Token | 值 | 用途 |
|---|---:|---|
| `brand.900` | `#070E57` | 深色品牌标题、深色图标 |
| `brand.800` | `#0B1B79` | 重点文字、活动标题 |
| `brand.700` | `#111FC2` | 品牌标题、链接强调 |
...
| Token | 值 | 说明 |
|---|---:|---|
| `text.primary` | `#101426` | 默认页面标题、卡片标题、关键值 |
| `text.brand` | `#0B1B79` | 大会品牌标题、可交互强调 |
...
- 标题：居中，使用 `title.page` + `text.primary`。
...
- 内容：标题最多 2 行，摘要最多 2 行；底部放日期和类型标签。
...
- 未读：标题 `600`、蓝紫圆点 + “未读”；已读：标题可保持 `500`，状态用灰色。
...
- `GuestCard.standard`：首页使用，固定三列；姓名 `16px / 600`，职位 `12–14px / 1–2 行`。
- `GuestCard.compact`：议程详情使用；`360px` 以上固定四列，姓名 `14px / 600`，职位最多两行；`320–359px` 在嘉宾不超过四位时使用两列，超过四位时使用 `GuestCarousel`。
...
- 标题最多一行，说明最多两行；右箭头是状态提示，不是唯一点击目标。
...
- 姓名使用 `title.section`；身份使用普通品牌标签，不用绿色成功色。
- 装饰背景不得降低姓名和身份信息的对比度。
...
- 状态颜色只用于图标、标题和标签，指标文字保持 `text.primary/secondary`。
...
- 标题和说明不得与箭头重叠；长文案允许说明两行。

### 5.27 分区链接标题 `SectionLinkHeader`

- 左侧显示分区标题，右侧显示“查看更多”文字或箭头；二者属于同一个 48px 高点击入口。
...
- 标题使用 `title.card`，正文使用 `body.md`；正文自然换行，不固定对话框高度。
...
| 页面标题颜色 | 黑色、深蓝混用 | AppBar 标题统一 `text.primary`；品牌/活动标题才使用 `text.brand` |
...
| 正文颜色 | 深蓝、黑色、灰蓝混用 | 标题 `text.primary`，正文 `text.secondary`，辅助 `text.tertiary` |
...
1. 只使用本文定义的颜色、间距、圆角、阴影、字阶和组件变体；新增 token 前先说明原因。
...
```css
@import "./design-system/tokens/v1.0/design-tokens.css";
...
```ts
import { designTokens } from './design-system/tokens/v1.0/design-tokens';
...
```bash
node design-system/tokens/v1.0/build-tokens.mjs
